...
num_bars = length(data_bars)
  ...
  all_bars_ = list()
  for (bar_num in 1:num_bars){
    all_bars_ = c(all_bars_, list((ggplot(data = data_bars[[bar_num]]$data,
                                          aes_string(x = data_bars[[bar_num]]$x_ax,
                                                     y = data_bars[[bar_num]]$y_ax,
                                                     group = "cat",
                                                     fill = "cat")) 
                                   + geom_bar(stat = "identity", width = lin_wid[bar_num], fill = colo[bar_num])
                                   + coord_flip()
                                   + theme_var 
                                   + facets
                                   + expand_limits(y = y_limits) 
                                   + ggtitle(title_[bar_num]) 
                                   + theme(plot.title = element_text(hjust = tit_loc)) 
                                   + x_y_lab_loc
                                   + legend_pos
                                   + scale_fill_manual(name = leg_tit,
                                                         labels = leg_nam[bar_num], values = colo[bar_num])
                                   + hline 
                                   + vline
    ))
    )
  ...
  bar_plot_ = multiplot(plotlist = all_bars_, cols = 1)
  bar_plot_ 
