...
##### Adding percent labels to df
library(scales)
library(dplyr)
df <- df %>% group_by(CW) %>% mutate(percentage = round(SPI/sum(SPI), 3))
df <- df %>% group_by(CW) %>% mutate(cum_percent = cumsum(percentage))
#############

df$cat = factor(df$cat, levels = c("bar2","bar3","bar1"))
...
colo = c("coral4", "gold", "mediumorchid4", "salmon1", "seagreen4", "peru")
...
############################### changing the order of the stacked bars
# set levels 
## 1. df$cat = factor(df$cat, levels = c("bar3","bar2","bar1"))
# make sure the legend is correctly reversed
## 2.  guide = guide_legend(reverse=TRUE) 
# reverse legend names
## 3. leg_nam = list("c", "b", "a")
  ...
  all_bars_ = (ggplot(data = df, 
                     aes(x = CW
                                ...
                                , fill = factor(cat)
                                    ...
                                    position = "fill", aes(order = -as.numeric(percentage))
                                    )
               )
